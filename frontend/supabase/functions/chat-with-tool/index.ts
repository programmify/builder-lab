...
serve(async (req) => {
  ...
  try {
    ...
    if (!OPENROUTER_API_KEY && provider !== 'lovable') {
      throw new Error("VITE_OPEN_ROUTER_API is not configured");
    }
...
- Avoid speculation. If unsure, ask a brief clarifying question.`;
  ...
  headers["X-Title"] = "Builders Lab by Programmify";
    ...
    const response = await fetch(targetUrl, {
      ...
      headers,
      body: JSON.stringify({
        // For Lovable, match original working implementation (Gemini 2.5 Flash)
        model: provider === 'lovable' ? "google/gemini-2.5-flash" : selectedModel,
        messages: [
